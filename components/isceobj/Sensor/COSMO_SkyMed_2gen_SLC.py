...
HDF5 = Component.Parameter(
    ...
    mandatory=True,
    intent='input',
    doc='CSG slc hdf5 input file'
)

from .Sensor import Sensor
class COSMO_SkyMed_2gen_SLC(Sensor):
    """
    ...
    logging_name = 'isce.Sensor.COSMO_SkyMed_2gen_SLC'
    family = 'cosmo_skymed_2gen_slc'

    def __init__(self,family='',name=''):
        super(COSMO_SkyMed_2gen_SLC,self).__init__(family if family else  self.__class__.family, name=name)
        self.frame = Frame()
        self.frame.configure()
        # Some extra processing parameters unique to CSG SLC (currently)
    ...
    def __setstate__(self,d):
        ...
        self.logger = logging.getLogger('isce.Sensor.COSMO_SkyMed_2gen_SLC')
        return
    ...
    def _populatePlatform(self, file):
        ...
        try:
            if file.attrs['Multi-Beam ID'].startswith('ES'):
                platform.setAntennaLength(16000.0/file['S01/IMG'].attrs['Line Time Interval'])
        except:
            pass
...
#        rangePixelSize = Const.c/(2*file['S01'].attrs['Sampling Rate'])
        rangePixelSize = file['S01/IMG'].attrs['Column Spacing']
        instrument.setRadarWavelength(file.attrs['Radar Wavelength'])
#        instrument.setPulseRepetitionFrequency(file['S01'].attrs['PRF'])
        instrument.setPulseRepetitionFrequency(1.0/file['S01/IMG'].attrs['Line Time Interval'])
...
#        instrument.setRangeSamplingRate(file['S01'].attrs['Sampling Rate'])
        instrument.setRangeSamplingRate(1.0/file['S01/IMG'].attrs['Column Time Interval'])

        incangle = 0.5*(file['S01/IMG'].attrs['Far Incidence Angle'] +
                 file['S01/IMG'].attrs['Near Incidence Angle'])
        instrument.setIncidenceAngle(incangle)


    def _populateFrame(self, file):

        rft = file['S01/IMG'].attrs['Zero Doppler Range First Time']
        ...
        relStart = file['S01/IMG'].attrs['Zero Doppler Azimuth First Time']
        relEnd = file['S01/IMG'].attrs['Zero Doppler Azimuth Last Time']
        ...
        self.frame.setPolarization(file.attrs['Polarization'])
        self.frame.setNumberOfLines(file['S01/IMG'].shape[0])
        self.frame.setNumberOfSamples(file['S01/IMG'].shape[1])
    ...
    def _populateExtras(self, file):
        ...
        self.dopplerRangeTime = file['S01'].attrs['Doppler Centroid vs Range Time Polynomial']
        self.dopplerAzimuthTime = file['S01'].attrs['Doppler Centroid vs Azimuth Time Polynomial']
        self.rangeRefTime = file['S01'].attrs['Range Polynomial Reference Time']
        self.azimuthRefTime = file['S01'].attrs['Azimuth Polynomial Reference Time']
        self.rangeFirstTime = file['S01/IMG'].attrs['Zero Doppler Range First Time']
        self.rangeLastTime = file['S01/IMG'].attrs['Zero Doppler Range Last Time']
        
        # get Doppler rate information, vs. azimuth first EJF 2015/00/05
        # guessing that same scale applies as for Doppler centroid 
        self.dopplerRateCoeffs = file['S01'].attrs['Doppler Rate vs Azimuth Time Polynomial']
        
    def extractImage(self):
        import os
        from ctypes import cdll, c_char_p
        extract_csg = cdll.LoadLibrary(os.path.dirname(__file__)+'/csg.so')
        inFile_c = c_char_p(bytes(self.hdf5, 'utf-8'))
        outFile_c = c_char_p(bytes(self.output, 'utf-8'))

        extract_csg.extract_csg_slc(inFile_c, outFile_c)
...
## CSG Doppler azimuth FM rate not yet implemented in reading section, set to zero for now
#
